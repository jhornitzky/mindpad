...
function PHPDiff($old,$new, $exploder) 
{
   # split the source text into arrays of lines
   $t1 = explode($exploder,$old);
   $x=array_pop($t1); 
   if ($x>'') $t1[]="$x\n\\ No newline at end of file";
   $t2 = explode($exploder,$new);
   $x=array_pop($t2); 
   if ($x>'') $t2[]="$x\n\\ No newline at end of file";
  ...
  $out[] = '';
  return join($exploder,$out);
}  

function PHPDiff2($old,$new, $exploder) 
{
   # split the source text into arrays of lines
   $t1 = explode($exploder,$old);
   $x=array_pop($t1); 
  ...
  foreach($actions as $act) {
    ...
    if ($op>0) {
      ...
      //if ($op==1) $out[] = "{$xstr}d{$y1}";
      //elseif ($op==3) $out[] = "{$xstr}c{$ystr}";
      //while ($x0<$x1) { $out[] = '< '.$t1[$x0]; $x0++; }   # deleted elems dont need these
      //if ($op==2) $out[] = "{$x1}a{$ystr}";
      //elseif ($op==3) $out[] = '---';
      while ($y0<$y1) { $out[] = '<span style="color:#EEE">&gt;</span> '.$t2[$y0]; $y0++; }   # added elems
    }
    $x1++; $x0=$x1;
    $y1++; $y0=$y1;
    $op=0;
